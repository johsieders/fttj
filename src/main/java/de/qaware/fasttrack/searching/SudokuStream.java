...
public class SudokuStream implements SearchProblemStream<SudokuStream.State, Step> {
    int[][] grid = new int[9][9];

    public SudokuStream(String input) {
        if (input != null) {
            int counter = 0;
    ...
    public Stream<Step> getSteps() {
        Iterator<Step> stepIterator = new getStepsIter();
        return Stream.generate(stepIterator::next).takeWhile(Objects::nonNull);
    }
    ...
    public class State {
        ...
        public String toString() {
            return SudokuStream.this.toString();
        }
    }
